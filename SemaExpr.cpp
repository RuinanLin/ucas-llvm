...
Sema::ActOnTensorSliceExpr(Scope *S, Expr *Base, SourceLocation LBLoc,
  ...
  // Handle any non-overload placeholder types in the base and index expressions.
  if(Base->getType()->isNonOverloadPlaceholderType())
  {
  ExprResult result=CheckPlaceholderExpr(Base);
  if(result.isInvalid()) return ExprError();
  ...
  // Perform default conversions.
  // converts functions and arrays to their respective pointers
  // and performs the lvalue-to-rvalue conversion.
  ExprResult result = DefaultFunctionArrayLvalueConversion(Base);
  if(result.isInvalid()) return ExprError();
  Base = result.get();
  QualType BaseTy= Base->getType();
  ...
  // Perform default conversions.
  // performs lvalue-to-rvalue conversion on the operand
  ...
  return new (Context)
		 TensorSliceExpr(Base, LowerBound, UpperBound, Step, 1, LColonLoc.isValid(), Base->getType(), VK_LValue, OK_Ordinary,
